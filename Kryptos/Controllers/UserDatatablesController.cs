...
namespace Kryptos.Controllers
{
    public class UserDatatablesController : Controller
    {
        ...
        private List<Organisation> GetMatchingOrganisationsForUserRole(UserLoginInformation currentUser)
        {
            ...
            {
                orgs = new List<Organisation> {_context.Organisations.Find(currentUser.Facility.OrganisationId)};
            }
            return orgs;
        }
            ...
            int[] pSelections)
        {
            ...
            else if (currentUser.IsOrganisationAdmin)
            {
                facilityMasters = (from facilityMaster in _context.FacilityMasters
                    where
                        pSelections.Any(
                            i =>
                                facilityMaster.OrganisationId.Equals(i) && i.Equals(currentUser.Facility.OrganisationId) &&
            ...
            else if (currentUser.IsFacilityAdmin)
            {
                ...
                {
                    (from facilityMaster in _context.FacilityMasters
                        where
                            pSelections.Any(
                                i =>
                                    facilityMaster.OrganisationId.Equals(i) &&
                                    i.Equals(currentUser.Facility.OrganisationId) &&
                                    facilityMaster.FacilityMasterId == currentUser.FacilityId &&
                                    facilityMaster.Status.Equals(1))
                        select facilityMaster).SingleOrDefault()
                };
            }
